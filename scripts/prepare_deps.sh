...
exec docker buildx build --platform=linux/amd64 --cache-from=type=local,src=$PROJECT_ROOT/cache --cache-to=type=local,dest=$PROJECT_ROOT/cache -f ${DIR}/Dockerfile --target=deps -t ${DOCKER_REGISTRY}${APP}:${DEPS_TAG} $PROJECT_ROOT
