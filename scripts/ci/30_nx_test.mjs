...
const DD_CIVISIBILITY_AGENTLESS_ENABLED =
...
const NODE_OPTIONS = `--max-old-space-size=8193 --unhandled-rejections=warn ${
...
try {
  console.log(
    `Running tests for ${AFFECTED_JOBS.join(
      ',',
    )} in parallel with ${MAX_JOBS} jobs`,
  )
  await runCommand(
    `nx run-many  --parallel=${MAX_JOBS} --target=test --projects=${AFFECTED_JOBS.join(
      ',',
    )}`,
  )
} catch (e) {
