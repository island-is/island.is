...
const GENERATE_HASH_GENERATED_FILES_SCRIPT = resolve(ROOT, ..."scripts/_hash-generated-files.sh".split('/'))
const ENV_HASHES_KEY = "CACHE_HASHES";
const HAS_HASH_KEYS = !!(process.env[ENV_HASHES_KEY] && process.env[ENV_HASHES_KEY] != "false")
...
const HASHES = HAS_HASH_KEYS ? JSON.parse(process.env[ENV_HASHES_KEY]) : {
    ...
    "App node_modules": `app-node-modules-${getPlatformString()}-${await getYarnLockHash()}-${await getPackageHash(MOBILE_APP_DIR)}-${await getNodeVersionString()}`,
...
async function writeToOutput(enabled = !!process.env.GITHUB_OUTPUT, hashes = HASHES, keys = HASHES_KEYS, file = process.env.GITHUB_OUTPUT ?? "") {
    if (!enabled) {
        return;
    }
    const values = Object.keys(hashes).map((key) => {
        return `${keys[key]}=${hashes[key]}`
    }).join('\n')
    await appendFile(file, values, 'utf-8')
    await appendFile(file, `\nkeys="${JSON.stringify(JSON.stringify(hashes))}"`, 'utf-8')
}
