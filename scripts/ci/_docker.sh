...
mkargs() {
  BUILD_ARGS=(
    ...
    --cache-from="type=s3,region=eu-west-1,bucket=${S3_DOCKER_CACHE_BUCKET},name=${APP}"
    --cache-to="type=s3,region=eu-west-1,bucket=${S3_DOCKER_CACHE_BUCKET},name=${APP},mode=max"
  )
