...
export type EcrContainer = AWSProps & { image: string } & {
...
async function getEcrPassword(props: AWSProps) {
  ...
  const sessionToken = await (async () => {
    if (props.AWS_SESSION_TOKEN) {
        return props.AWS_SESSION_TOKEN
    }
    const sessionContainer = await container
      .withExec(
        [
          'aws',
          'sts',
          'get-session-token',
          '--duration-seconds',
          AWS_SESSION_TIMEOUT_SEC.toString(),
        ],
        { redirectStdout: tmpFile },
      )
      .sync()
    ...
    return dag.setSecret(
      ...
      sessionJSON.Credentials.SessionToken,
    )
  })()
