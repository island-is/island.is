...
if (hasPermission) {
    info(`User ${labelOwner} has sufficient permissions to set label: ${TEST_EVERYTHING_LABEL}`);
    setTestEverything(true);
    process.exit(0);
}

setTestEverything(false);
setFailed(`User ${labelOwner} does not have sufficient permissions to set label: ${TEST_EVERYTHING_LABEL}.`);
process.exit(1);
