...
import { getPlatformString, getYarnLockHash, getPackageHash, getNodeVersionString, getGeneratedFileHash, folderSizeIsEqualOrGreaterThan, runCommand, fileSizeIsEqualOrGreaterThan } from "./_utils.mjs";
...
export const enableMobileNodeModules = process.env.ENABLE_MOBILE_NODE_MODULES === 'true';
export const enableGeneratedFiles = process.env.ENABLE_GENERATED_FILES === 'true';
export const enableCypress = process.env.ENABLE_CYPRESS === 'true';
export const cypressPath = process.env.CYPRESS_PATH;
export const keys = process.env[ENV_KEYS] ? JSON.parse(process.env[ENV_KEYS]) : {};
...
export const caches = [
    {
        ...
        hash: async () => keys['node-modules'] ?? `node-modules-${HASH_VERSION}-${getPlatformString()}-${await getYarnLockHash()}-${await getPackageHash()}-${await getNodeVersionString()}`,
    ...
    },
    {
        ...
        hash: async () => keys['mobile-node-modules'] ?? `app-node-modules-${HASH_VERSION}-${getPlatformString()}-${await getYarnLockHash()}-${await getPackageHash(MOBILE_APP_DIR)}-${await getNodeVersionString()}`,
    ...
    },
    {
        ...
        hash: async () => keys['generated-files'] ?? `generated-files-${HASH_VERSION}-${getPlatformString()}-${await getGeneratedFileHash()}`,
    ...
    },
    {
        ...
        hash: async () => keys['cypress-cache'] ?? `cypress-cache-${HASH_VERSION}-${getPlatformString()}-${await getYarnLockHash()}-${await getPackageHash()}-${await getNodeVersionString()}`,
        name: 'Cache Cypress',
        id: 'cypress-cache',
        path: cypressPath || '',
