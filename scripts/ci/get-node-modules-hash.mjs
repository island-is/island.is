...
process.stdout.write(`${getPlatformString()}-${await getYarnLockHash()}-${await getPackageHash()}-${await getNodeVersionString()}`);
...
async function getPackageHash(keys = ["resolutions", "dependencies", "devDependencies"]) {
    const content = await getPackageJSON()
