...
function getDirtyFiles() {
  ...
  const stat = runCapture('git', ['diff', '--name-only', '--', `"${absPath}"`], { cwd: repoRoot }).split('\n').filter(Boolean).map((e) => path.resolve(repoRoot, e)).filter((e) => !ignoredFiles.includes(e));
  return stat;
}
 ...
 */
function usage() {
  console.error('Usage: node git-check-dirty.mjs <relative_path> <action> [owner]');
  process.exit(1);
}
...
function runCapture(cmd, argv, opts = {}) {
  return execSync([cmd, ...argv].join(' '), { encoding: 'utf8', ...opts }).trim();
}
