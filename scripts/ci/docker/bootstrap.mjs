...
for (const [chart, deployment] of Object.entries(bootstrap)) {
    for (const [type, name] of Object.entries(deployment)) {
        ...
        for (const file of files) {
            ...
            const namespaceToAdd = yamlContent && typeof yamlContent === 'object' && 'namespace' in yamlContent ? yamlContent?.namespace : null;
            if (namespaceToAdd) {
                namespacesToAdd[name] ??= [];
                namespacesToAdd[name] = Array.from(new Set([...namespacesToAdd[name], namespaceToAdd]));
            }
        }
    }
}
