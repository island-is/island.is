...
export class AuditTrailService {
  ...
  initTrail(options: AuditTrailOptions) {
    if (options.useLogger) {
      this.logger.info('Using general logger for audit trail')
  ...
  audit(userId: string, action: AuditedAction, caseIds: string[]) {
    if (!this.trail) {
      throw new ReferenceError('Audit trail has not been initialized1')
    }
