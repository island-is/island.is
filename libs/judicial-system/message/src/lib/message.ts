import type { User } from '@island.is/judicial-system/types'

export enum MessageType {
  DELIVERY_TO_COURT_PROSECUTOR = 'DELIVERY_TO_COURT_PROSECUTOR',
  DELIVERY_TO_COURT_DEFENDANT = 'DELIVERY_TO_COURT_DEFENDANT',
  DELIVERY_TO_COURT_REQUEST_DEFENDANT = 'DELIVERY_TO_COURT_REQUEST_DEFENDANT',
  DELIVERY_TO_COURT_INDICTMENT = 'DELIVERY_TO_COURT_INDICTMENT',
  DELIVERY_TO_COURT_INDICTMENT_INFO = 'DELIVERY_TO_COURT_INDICTMENT_INFO',
  DELIVERY_TO_COURT_INDICTMENT_COURT_ROLES = 'DELIVERY_TO_COURT_INDICTMENT_COURT_ROLES',
  DELIVERY_TO_COURT_INDICTMENT_ARRAIGNMENT_DATE = 'DELIVERY_TO_COURT_INDICTMENT_ARRAIGNMENT_DATE',
  DELIVERY_TO_COURT_INDICTMENT_DEFENDANT = 'DELIVERY_TO_COURT_INDICTMENT_DEFENDANT',
  DELIVERY_TO_COURT_INDICTMENT_CIVIL_CLAIMANT = 'DELIVERY_TO_COURT_INDICTMENT_CIVIL_CLAIMANT',
  DELIVERY_TO_COURT_INDICTMENT_CANCELLATION_NOTICE = 'DELIVERY_TO_COURT_INDICTMENT_CANCELLATION_NOTICE',
  DELIVERY_TO_COURT_INDICTMENT_CONCLUSION = 'DELIVERY_TO_COURT_INDICTMENT_CONCLUSION',
  DELIVERY_TO_COURT_CASE_FILE = 'DELIVERY_TO_COURT_CASE_FILE',
  DELIVERY_TO_COURT_CASE_FILES_RECORD = 'DELIVERY_TO_COURT_CASE_FILES_RECORD',
  DELIVERY_TO_COURT_REQUEST = 'DELIVERY_TO_COURT_REQUEST',
  DELIVERY_TO_COURT_SUBPOENA = 'DELIVERY_TO_COURT_SUBPOENA',
  DELIVERY_TO_COURT_SERVICE_CERTIFICATE = 'DELIVERY_TO_COURT_SERVICE_CERTIFICATE',
  DELIVERY_TO_COURT_COURT_RECORD = 'DELIVERY_TO_COURT_COURT_RECORD',
  DELIVERY_TO_COURT_COURT_RECORD_WORKING_DOCUMENT = 'DELIVERY_TO_COURT_COURT_RECORD_WORKING_DOCUMENT',
  DELIVERY_TO_COURT_SIGNED_COURT_RECORD = 'DELIVERY_TO_COURT_SIGNED_COURT_RECORD',
  DELIVERY_TO_COURT_SIGNED_RULING = 'DELIVERY_TO_COURT_SIGNED_RULING',
  DELIVERY_TO_COURT_CASE_CONCLUSION = 'DELIVERY_TO_COURT_CASE_CONCLUSION',
  DELIVERY_TO_COURT_OF_APPEALS_RECEIVED_DATE = 'DELIVERY_TO_COURT_OF_APPEALS_RECEIVED_DATE',
  DELIVERY_TO_COURT_OF_APPEALS_ASSIGNED_ROLES = 'DELIVERY_TO_COURT_OF_APPEALS_ASSIGNED_ROLES',
  DELIVERY_TO_COURT_OF_APPEALS_CASE_FILE = 'DELIVERY_TO_COURT_OF_APPEALS_CASE_FILE',
  DELIVERY_TO_COURT_OF_APPEALS_CONCLUSION = 'DELIVERY_TO_COURT_OF_APPEALS_CONCLUSION',
  // The case update and the documents of a completed case are delivered separately,
  // so that each of them can be retried on its own
  DELIVERY_TO_POLICE_CASE = 'DELIVERY_TO_POLICE_CASE', // Only updates the case
  DELIVERY_TO_POLICE_INDICTMENT_CASE = 'DELIVERY_TO_POLICE_INDICTMENT_CASE', // Only updates the case
  DELIVERY_TO_POLICE_INDICTMENT = 'DELIVERY_TO_POLICE_INDICTMENT',
  DELIVERY_TO_POLICE_REQUEST = 'DELIVERY_TO_POLICE_REQUEST',
  DELIVERY_TO_POLICE_COURT_RECORD = 'DELIVERY_TO_POLICE_COURT_RECORD',
  DELIVERY_TO_POLICE_CUSTODY_NOTICE = 'DELIVERY_TO_POLICE_CUSTODY_NOTICE',
  DELIVERY_TO_POLICE_CASE_FILE = 'DELIVERY_TO_POLICE_CASE_FILE',
  DELIVERY_TO_POLICE_CASE_FILES_RECORD = 'DELIVERY_TO_POLICE_CASE_FILES_RECORD',
  DELIVERY_TO_POLICE_SIGNED_COURT_RECORD = 'DELIVERY_TO_POLICE_SIGNED_COURT_RECORD',
  DELIVERY_TO_POLICE_SIGNED_RULING = 'DELIVERY_TO_POLICE_SIGNED_RULING',
  DELIVERY_TO_POLICE_APPEAL = 'DELIVERY_TO_POLICE_APPEAL',
  DELIVERY_TO_POLICE_SERVICE_CERTIFICATE = 'DELIVERY_TO_POLICE_SERVICE_CERTIFICATE',
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA = 'DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA', // This delivers the subpoena to the centralized file service to be served
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA_REVOCATION = 'DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA_REVOCATION',
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_VERDICT = 'DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_VERDICT',
  NOTIFICATION = 'NOTIFICATION',
  APPEAL_CASE_NOTIFICATION = 'APPEAL_CASE_NOTIFICATION',
  INDICTMENT_CASE_NOTIFICATION = 'INDICTMENT_CASE_NOTIFICATION',
  SUBPOENA_NOTIFICATION = 'SUBPOENA_NOTIFICATION',
  INSTITUTION_NOTIFICATION = 'INSTITUTION_NOTIFICATION',
  DEFENDANT_NOTIFICATION = 'DEFENDANT_NOTIFICATION',
  CIVIL_CLAIMANT_NOTIFICATION = 'CIVIL_CLAIMANT_NOTIFICATION',
  NOTIFICATION_DISPATCH = 'NOTIFICATION_DISPATCH',
  EVENT_NOTIFICATION_DISPATCH = 'EVENT_NOTIFICATION_DISPATCH',
}

export const messageEndpoint: { [key in MessageType]: string } = {
  DELIVERY_TO_COURT_PROSECUTOR: 'deliverProsecutorToCourt',
  DELIVERY_TO_COURT_DEFENDANT: 'deliverDefendantToCourt',
  DELIVERY_TO_COURT_REQUEST_DEFENDANT: 'deliverRequestDefendantToCourt',
  DELIVERY_TO_COURT_INDICTMENT: 'deliverIndictmentToCourt',
  DELIVERY_TO_COURT_INDICTMENT_COURT_ROLES:
    'deliverIndictmentCourtRolesToCourt',
  DELIVERY_TO_COURT_INDICTMENT_ARRAIGNMENT_DATE:
    'deliverIndictmentArraignmentDateToCourt',
  DELIVERY_TO_COURT_INDICTMENT_INFO: 'deliverIndictmentInfoToCourt',
  DELIVERY_TO_COURT_INDICTMENT_DEFENDANT: 'deliverIndictmentDefendantToCourt',
  DELIVERY_TO_COURT_INDICTMENT_CIVIL_CLAIMANT:
    'deliverIndictmentCivilClaimantToCourt',
  DELIVERY_TO_COURT_INDICTMENT_CANCELLATION_NOTICE:
    'deliverIndictmentCancellationNoticeToCourt',
  DELIVERY_TO_COURT_INDICTMENT_CONCLUSION: 'deliverIndictmentConclusionToCourt',
  DELIVERY_TO_COURT_CASE_FILE: 'deliverCaseFileToCourt',
  DELIVERY_TO_COURT_CASE_FILES_RECORD: 'deliverCaseFilesRecordToCourt',
  DELIVERY_TO_COURT_REQUEST: 'deliverRequestToCourt',
  DELIVERY_TO_COURT_SUBPOENA: 'deliverSubpoenaToCourt',
  DELIVERY_TO_COURT_SERVICE_CERTIFICATE: 'deliverServiceCertificateToCourt',
  DELIVERY_TO_COURT_COURT_RECORD: 'deliverCourtRecordToCourt',
  DELIVERY_TO_COURT_COURT_RECORD_WORKING_DOCUMENT:
    'deliverCourtRecordWorkingDocumentToCourt',
  DELIVERY_TO_COURT_SIGNED_COURT_RECORD: 'deliverSignedCourtRecordToCourt',
  DELIVERY_TO_COURT_SIGNED_RULING: 'deliverSignedRulingToCourt',
  DELIVERY_TO_COURT_CASE_CONCLUSION: 'deliverCaseConclusionToCourt',
  DELIVERY_TO_COURT_OF_APPEALS_RECEIVED_DATE:
    'deliverReceivedDateToCourtOfAppeals',
  DELIVERY_TO_COURT_OF_APPEALS_ASSIGNED_ROLES:
    'deliverAssignedRolesToCourtOfAppeals',
  DELIVERY_TO_COURT_OF_APPEALS_CASE_FILE: 'deliverCaseFileToCourtOfAppeals',
  DELIVERY_TO_COURT_OF_APPEALS_CONCLUSION: 'deliverConclusionToCourtOfAppeals',
  DELIVERY_TO_POLICE_CASE: 'deliverCaseToPolice',
  DELIVERY_TO_POLICE_INDICTMENT_CASE: 'deliverIndictmentCaseToPolice',
  DELIVERY_TO_POLICE_INDICTMENT: 'deliverIndictmentToPolice',
  DELIVERY_TO_POLICE_REQUEST: 'deliverRequestToPolice',
  DELIVERY_TO_POLICE_COURT_RECORD: 'deliverCourtRecordToPolice',
  DELIVERY_TO_POLICE_CUSTODY_NOTICE: 'deliverCustodyNoticeToPolice',
  DELIVERY_TO_POLICE_CASE_FILE: 'deliverCaseFileToPolice',
  DELIVERY_TO_POLICE_CASE_FILES_RECORD: 'deliverCaseFilesRecordToPolice',
  DELIVERY_TO_POLICE_SIGNED_COURT_RECORD: 'deliverSignedCourtRecordToPolice',
  DELIVERY_TO_POLICE_SIGNED_RULING: 'deliverSignedRulingToPolice',
  DELIVERY_TO_POLICE_APPEAL: 'deliverAppealToPolice',
  DELIVERY_TO_POLICE_SERVICE_CERTIFICATE: 'deliverServiceCertificateToPolice',
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA:
    'deliverSubpoenaToNationalCommissionersOffice',
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_SUBPOENA_REVOCATION:
    'deliverSubpoenaRevocationToNationalCommissionersOffice',
  DELIVERY_TO_NATIONAL_COMMISSIONERS_OFFICE_VERDICT:
    'deliverVerdictToNationalCommissionersOffice',
  NOTIFICATION: 'notification',
  APPEAL_CASE_NOTIFICATION: 'appealCaseNotification',
  SUBPOENA_NOTIFICATION: 'subpoenaNotification',
  INSTITUTION_NOTIFICATION: 'institutionNotification',
  DEFENDANT_NOTIFICATION: 'defendantNotification',
  CIVIL_CLAIMANT_NOTIFICATION: 'civilClaimantNotification',
  INDICTMENT_CASE_NOTIFICATION: 'indictmentCaseNotification',
  NOTIFICATION_DISPATCH: 'notification/dispatch',
  EVENT_NOTIFICATION_DISPATCH: 'eventNotification/dispatch',
}

export type Message = {
  type: MessageType
  user?: User
  caseId?: string
  elementId?: string | string[]
  body?: { [key: string]: unknown }
  numberOfRetries?: number
  nextRetry?: number
}
