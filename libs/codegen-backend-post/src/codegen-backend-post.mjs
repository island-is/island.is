...
import { IGNORE_PROJECTS, POST_BACKEND_TARGETS, PROJECTJSON } from './const.mjs';
import { readFile } from 'fs/promises';

const projectJSON = JSON.parse(await readFile(PROJECTJSON, 'utf-8'));
const currentProjects = Array.from(new Set((await Promise.all(POST_BACKEND_TARGETS.map(async (target) => {
...
const definedProjects = projectJSON.implicitDependencies ?? [];
