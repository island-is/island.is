...
import { primarySchoolMessages, statesMessages } from './messages'
import { CodeOwners } from '@island.is/shared/constants'
import {
  determineNameFromApplicationAnswers,
  getApplicationAnswers,
  hasForeignLanguages,
} from './primarySchoolUtils'

const PrimarySchoolTemplate: ApplicationTemplate<
  ApplicationContext,
  ApplicationStateSchema<Events>,
  Events
> = {
  type: ApplicationTypes.PRIMARY_SCHOOL,
  name: determineNameFromApplicationAnswers,
  codeOwner: CodeOwners.Deloitte,
  institution: primarySchoolMessages.shared.institution,
  translationNamespaces: ApplicationConfigurations.PrimarySchool.translation,
  dataSchema,
          ...
          roles: [
            {
              id: Roles.APPLICANT,
              formLoader: () =>
                import('../forms/PrimarySchoolForm/index').then((val) =>
                  Promise.resolve(val.PrimarySchoolForm),
                ),
              actions: [
                {
                  event: DefaultEvents.SUBMIT,
...
export default PrimarySchoolTemplate
