import { defineMessages } from 'react-intl'

export const specialProvisions = defineMessages({
  subSectionName: {
    id: 'ra.application:specialProvisions.subSectionName',
    defaultMessage: 'Sérákvæði',
    description: 'Special provisions sub section name',
  },
  pageTitle: {
    id: 'ra.application:specialProvisions.pageTitle',
    defaultMessage: 'Nánari lýsing og sérákvæði',
    description: 'Special provisions page title',
  },
  pageDescription: {
    id: 'ra.application:specialProvisions.pageDescription',
    defaultMessage:
      'Hér má taka fram hvað fylgir húsnæðinu, hvort húsreglur séu til staðar eða önnur ákvæði sem fylgja samningnum. Athugið að þau ákvæði sem eru andstætt húsaleigulögum munu ekki hafa gildi í túlkun leigusamningsins. Sjá nánar hér.',
    description: 'Special provisions page description',
  },
  housingDescriptionTitle: {
    id: 'ra.application:specialProvisions.housingDescriptionTitle',
    defaultMessage: 'Lýsing á húsnæðinu og því sem með fylgir ',
    description: 'Special provisions housing description title',
  },
  housingDescriptionTitleTooltip: {
    id: 'ra.application:specialProvisions.housingDescriptionTitleTooltip',
    defaultMessage: 'Þetta er tooltip fyrir titilinn',
    description: 'Special provisions housing description title tooltip',
  },
  housingDescriptionInputLabel: {
    id: 'ra.application:specialProvisions.housingDescriptionInputLabel',
    defaultMessage: 'Lýsing á húsnæði',
    description: 'Special provisions housing description input',
  },
  housingDescriptionInputPlaceholder: {
    id: 'ra.application:specialProvisions.housingDescriptionInputPlaceholder',
    defaultMessage: 'Skrifaðu hér lýsingu á húsnæðinu ',
    description: 'Special provisions housing description input placeholder',
  },
  housingRulesDescriptionTitle: {
    id: 'ra.application:specialProvisions.housingRulesDescriptionTitle',
    defaultMessage: 'Sérákvæði eða húsreglur',
    description: 'Special provisions housing rules title',
  },
  housingRulesDescriptionTitleTooltip: {
    id: 'ra.application:specialProvisions.housingRulesDescriptionTitleTooltip',
    defaultMessage: 'Þetta er tooltip fyrir titilinn',
    description: 'Special provisions housing rules title tooltip',
  },
  housingRulesDescriptionInputLabel: {
    id: 'ra.application:specialProvisions.housingRulesDescriptionInputLabel',
    defaultMessage: 'Sérákvæði',
    description: 'Special provisions housing rules input',
  },
  housingRulesDescriptionInputPlaceholder: {
    id: 'ra.application:specialProvisions.housingRulesDescriptionInputPlaceholder',
    defaultMessage: 'Skrifaðu hér allt sem á við',
    description: 'Special provisions housing rules input placeholder',
  },
})
