...
export const CoOwner: FC<FieldBaseProps> = ({
  ...
  const [getVehicleInformation, { loading }] = useLazyQuery(
    ...
    `,
    {
      onCompleted: (result) => {
        ...
        if (data?.operators !== oldOperators) {
  ...
  }, [coOwners, oldOperators])

  return coOwners.length > 0 || oldOperators.length > 0 ? (
    <>
      {coOwners.length > 0 ? (
        <Box>
      ...
      {/* TODOx ATH þetta virkar ekki, vistast ekki í answers... */}
      {oldOperators.length > 0
        ? oldOperators.map(
            (oldOperator: OldOperatorInformation, index: number) => (
              <>
                <input
                  type="hidden"
                  value={oldOperator.nationalId || ''}
                  ref={register({ required: true })}
                  name={`oldOperators[${index}].nationalId`}
                />
                <input
                  type="hidden"
                  value={oldOperator.name || ''}
                  ref={register({ required: true })}
                  name={`oldOperators[${index}].name`}
                />
              </>
            ),
          )
