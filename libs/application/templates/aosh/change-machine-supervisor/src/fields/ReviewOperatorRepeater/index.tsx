  ...
  return (
    <Box>
      <Text variant="h2" marginBottom={1}>
        {formatMessage(information.labels.supervisor.title)}
      </Text>
      <Text marginBottom={5}>
        {formatMessage(information.labels.supervisor.description)}
      </Text>
          ...
          {buyerOperator?.nationalId &&
            buyerOperator?.nationalId?.trim() !== '' && (
              <Button variant="text" onClick={handleClear}>
                {formatMessage(information.labels.supervisor.remove)}
              </Button>
            )}
        </Box>
        <NationalIdWithName
          {...props}
          id="buyerOperator"
          customNameLabel={formatMessage(information.labels.supervisor.name)}
          customNationalIdLabel={formatMessage(
            information.labels.supervisor.nationalId,
          )}
        ...
        <GridRow>
          <GridColumn span={['1/1', '1/1', '1/2']} paddingTop={2}>
            <InputController
              ...
              name={'email'}
              type="email"
              label={formatMessage(information.labels.supervisor.email)}
              error={
          ...
          <GridColumn span={['1/1', '1/1', '1/2']} paddingTop={2}>
            <InputController
              ...
              type="tel"
              format="###-####"
              label={formatMessage(information.labels.supervisor.phone)}
              error={
