...
import { DEFAULT_JOB_FACTORS, type JobFactor, type PersonalFactor, type SubCriterion } from '../../lib/constants'
...
export const SubCriteriaEditor: FC<React.PropsWithChildren<FieldBaseProps>> = ({
  ...
  const parsedPersonalCriteria = parsedCriteria.filter((c) => c.type === 'PERSONAL')
    ...
    <Box>
      <Stack space={4}>
        <Box>
          ...
          <Stack space={3}>
            {jobFactors.map((factor, i) => {
              const parsedCriterion = parsedCriteria.find((c) => c.type === factor.type)
              return (
                <CriterionPanel
                  key={factor.type}
        ...
        {personalFactors.length > 0 && (
          <Box>
            <Text variant="h4" marginBottom={1}>
              {formatMessage(messages.report.subCriteria.personalFactorGroupTitle)}
            </Text>
            <Text marginBottom={3}>
              {formatMessage(messages.report.subCriteria.personalFactorGroupIntro)}
            </Text>
            <Stack space={3}>
              {personalFactors.map((factor, i) => (
                <CriterionPanel
                  ...
                  parsedSubCriteria={parsedPersonalCriteria[i]?.subCriteria ?? []}
                  parsedSalaryReportDate={parsedSalaryReportDate}
                  startExpanded={i === 0}
                />
              ))}
