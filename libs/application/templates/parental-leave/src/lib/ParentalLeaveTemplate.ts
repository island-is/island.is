...
import { applicationSystemMessages } from '@island.is/application/translations'
...
> => {
  return {
    ...
    // name: 'asa:parentalLeave.name',
    name: formatMessage(
      applicationSystemMessages.parentalLeave.name.defaultMessage,
    ),
    // dataSchema: () => dataSchema('some string'),
    dataSchema,
    stateMachineConfig: {
