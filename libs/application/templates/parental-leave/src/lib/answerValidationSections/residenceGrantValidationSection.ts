...
import { getApplicationAnswers, residentGrantIsOpenForApplication } from "../parentalLeaveUtils"
...
export const residenceGrantValidationSection = (
  ...
  if (validatePeriodResidenceGrant(
    `${dateOfBirth}`,
    hasMultipleBirths,
...
export const validatePeriodResidenceGrant = (
    ...
    if (result >= 14) return true
    if (result >= 28 && multipleBirths === 'yes') return true
    return false
  }
