...
import { AnswerValidationConstants, PARENTAL_GRANT_STUDENTS, SINGLE, UnEmployedBenefitTypes, YES } from "../../constants";
...
export const fileUploadValidationSection = (newAnswer: unknown, application: Application) => {
  ...
  if (isSelfEmployed === YES && obj.selfEmployedFile) {
    if (isEmpty((obj as { selfEmployedFile: unknown[] }).selfEmployedFile))
      return buildError(errorMessages.requiredAttachment, 'selfEmployedFile', FILEUPLOAD)
  ...
  if (applicationType === PARENTAL_GRANT_STUDENTS && obj.studentFile) {
    if (isEmpty((obj as { studentFile: unknown[] }).studentFile))
      return buildError(errorMessages.requiredAttachment, 'studentFile', FILEUPLOAD)
    return undefined
  }

  if (otherParent === SINGLE && obj.singleParent) {
    if (isEmpty((obj as { singleParent: unknown[] }).singleParent))
      return buildError(errorMessages.requiredAttachment, 'singleParent', FILEUPLOAD)
  ...
  if (isRecivingUnemploymentBenefits) {
    if (
      ...
      if (isEmpty((obj as { benefitsFile: unknown[] }).benefitsFile))
        return buildError(errorMessages.requiredAttachment, 'benefitsFile', FILEUPLOAD)
