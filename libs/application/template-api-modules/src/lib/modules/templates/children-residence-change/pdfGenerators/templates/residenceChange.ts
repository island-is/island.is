...
import { DistrictCommissionerLogo } from '../assets/logo'
...
export async function generateResidenceChangePdf(application: CRCApplication) {
  ...
  addValue(
    durationType === TEMPORARY && durationDate
      ...
      : 'Samningurinn er ótímabundinn þar til barnið hefur náð 18 ára aldri.',
    doc,
    PdfConstants.NORMAL_FONT,
    PdfConstants.LARGE_LINE_GAP,
  )
  ...
  addValue(
    selectChildSupportPayment === 'agreement'
      ? 'Samkomulag er um framfærslu, ekki verður af greiðslu hefðbundins meðlags.' // TODO: Get correct text from syslumenn
