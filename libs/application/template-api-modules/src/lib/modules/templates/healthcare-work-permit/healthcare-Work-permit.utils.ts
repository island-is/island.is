...
export const mapSecondarySchoolStudentTrack = (innaDiplomas: InlineResponse200 | null): StudentGraduations[] => {
    if (!innaDiplomas || !innaDiplomas?.items) return [];

    return innaDiplomas.items.map(({ diplomaDate, diplomaCode, diplomaName, organisation }) => ({
...
export const mapUniversityStudentTracks = (studentTracks: StudentTrackDto[] | null): StudentGraduations[] => {
