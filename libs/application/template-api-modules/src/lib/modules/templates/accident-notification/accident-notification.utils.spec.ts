...
import { AccidentNotificationAttachment, AttachmentTypeEnum } from './types/attachments';
...
describe('applictionAnswersToXml', () => {
    it.each([
    ...
    ])('should map correctly for a person reporting an injury for themselves, injured in sports, rescue work or studies, with no employer involved', (accidentType) => {
        ...
        const result = utils.applictionAnswersToXml(answers as AccidentNotificationAnswers, attachments)
    ...
    it.each([
    ...
    ])('should map correctly for a person reporting an injury for themselves, injured in at home or at work, with employer involved', (data) => {
        const attachments = getDefaultAttachments()
        const answers = getFullBasicAnswers()
        ...
        if(data.type === AccidentTypeEnum.HOMEACTIVITIES) {
            ...
            expect(result.includes('homeAccidentAddress123PortalCode100')).toBeTruthy()
            ...
            if(data.fisherman) {
                ...
                expect(result.includes('description of machine that messed someone up')).toBeTruthy()
            }
    ...
    it.each([
        {for: WhoIsTheNotificationForEnum.ME, someoneElse: false, shouldHaveCompanyInfo: false},
        {for: WhoIsTheNotificationForEnum.POWEROFATTORNEY, someoneElse: true, shouldHaveCompanyInfo: false},
        {for: WhoIsTheNotificationForEnum.JURIDICALPERSON, someoneElse: true, shouldHaveCompanyInfo: true},
        {for: WhoIsTheNotificationForEnum.CHILDINCUSTODY, someoneElse: true, shouldHaveCompanyInfo: false}
    ])('should map correctly for a person reporting an injury for themselves, someone else, juridical person, or their child, with no employer involved', (data) => {
        const attachments = getDefaultAttachments()
        const answers = getFullBasicAnswers()
        ...
        if(data.someoneElse) {
            ...
            ? expect(result.includes('Some Kid') && !result.includes('Fullername Fullername')).toBeTruthy()
            : expect(result.includes('Fullername Fullername') && !result.includes('Some Kid')).toBeTruthy()
