...
export class PrimarySchoolService extends BaseTemplateApiService {
  constructor(
    ...
    super(ApplicationTypes.PRIMARY_SCHOOL)
  }
  ...
  async sendApplication({ auth, application }: TemplateApiModuleActionProps) {
    const primarySchoolDTO = transformApplicationToPrimarySchoolDTO(application)

    return await this.friggClientService.sendApplication(auth, primarySchoolDTO)
  }
}
