import { Application } from 'libs/application/core/src'
import get from 'lodash/get'
import { Agency, ComplaintDto, TargetOfComplaint } from './models'
import { OnBehalf, subjectOfComplaintValueLabelMapper, onBehalfValueLabelMapper, SubjectOfComplaint } from '@island.is/application/templates/data-protection-complaint'
...
const extractAnswer = <T>(
  ...
  if (defaultValue === undefined && typeof value === 'undefined') {
    throw new Error(
      `ComplaintDto.extractAnswer: missing value for ${path}`,
    )
  }
...
export const getComplaintTargets = (application: Application): TargetOfComplaint[] => {
  const firstTarget = extractAnswer<TargetOfComplaint>(application.answers, 'complainee')
  const otherTargets = extractAnswer<TargetOfComplaint[]>(application.answers, 'additionalComplainees', [])
...
export const getAndFormatSubjectsOfComplaint = (application: Application): string[] => {
  const other = extractAnswer<SubjectOfComplaint[]>(application.answers, 'subjectOfComplaint.other')
  const authorities = extractAnswer<SubjectOfComplaint[]>(application.answers, 'subjectOfComplaint.authorities')
  const useOfPersonalInformation = extractAnswer<SubjectOfComplaint[]>(application.answers, 'subjectOfComplaint.useOfPersonalInformation')

  const enums = [...other, ...authorities, ...useOfPersonalInformation]

  const somethingElse = extractAnswer<string>(application.answers, 'subjectOfComplaint.somethingElse') ?? ''

  return [...enums.map((subject) => {
