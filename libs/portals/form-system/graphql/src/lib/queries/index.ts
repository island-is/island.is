export { GET_ADDRESS_BY_NATIONALID } from './getAddress'
export { GET_ALL_APPLICATIONS } from './getAllApplications'
export { GET_APPLICATION } from './getApplication'
export { GET_COMPANY_BY_NATIONALID } from './getCompany'
export { GET_FORM } from './getForm'
export { GET_FORMS } from './getForms'
export { GET_APPLICATION_JSON_SAMPLE } from './getApplicationJson'
export { GET_GOOGLE_TRANSLATION } from './getGoogleTranslation'
export { GET_NAME_BY_NATIONALID } from './getName'
export { GET_ORGANIZATION_ADMIN } from './getOrganizationAdmin'
export { GET_ORGANIZATIONS } from './getOrganizations'
export { GET_ORGANIZATION_TITLE } from './getOrganizationTitle'
export { GET_ORGANIZATION_ZENDESK_INSTANCE } from './getOrganizationZendeskInstance'
export { GET_PAYMENT_CATALOG } from './getPaymentCatalog'
export { PAYMENT_STATUS } from './getPaymentStatus'
export { GET_PERSON } from './getPerson'
export { GET_QNA } from './getQNA'
export { GET_COMPANY } from './getRskCompany'
export { IDENTITY_QUERY } from './identityQuery'
export { GET_VEHICLES_LIST_V3 } from './getVehiclesListV3'
export { GET_PUBLIC_VEHICLE_SEARCH } from './getPublicVehicleSearch'
export { GET_REAL_ESTATE_PROPERTIES } from './getRealEstateProperties'
export { GET_REAL_ESTATE_PROPERTY } from './gerRealEstateProperty'
