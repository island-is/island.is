...
export const LandRegistry = ({ landRegistry, loading, error }: Props) => {
  ...
  const renderExpandedRow = (row: Row<FarmerLandRegistryEntry>) => {
    ...
    if (!properties.length) return <EmptyTable message={formatMessage(m.noData)} />
    return (
    <T.Table>
      <T.Head>
        <T.Row>
          <T.HeadData text={{ variant: 'small', fontWeight: 'semiBold' }}>{formatMessage(fm.ownershipType)}</T.HeadData>
          <T.HeadData text={{ variant: 'small', fontWeight: 'semiBold' }}>{formatMessage(fm.usage)}</T.HeadData>
