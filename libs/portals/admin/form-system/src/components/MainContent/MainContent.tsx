...
export default function MainContent() {
  const { control, controlDispatch, updateActiveItem, setFocus, focus } = useContext(ControlContext)
  ...
  return (
    <Box padding={2}>
      ...
      {activeItem.type === 'Input' ? (<InputContent />)
        : activeItem.type === 'Step' &&
          (activeItem.data as FormSystemStep).type === 'BaseSetting' ? (<BaseSettings />)
          : activeItem.type === 'Step' &&
            (activeItem.data as FormSystemStep).type === 'Forsendur' ? (<Premises />)
            : activeItem.type === 'Step' && (activeItem.data as FormSystemStep).type === 'Aðilar' ? <RelevantParties />
              : openPreview ? (
