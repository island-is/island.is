...
interface IdpProviderConfirmModalProps {
  target: IdpProviderRow
  configuredEnvironments: AuthAdminEnvironment[]
  onConfirm: (name: string, environments: AuthAdminEnvironment[]) => void
  onClose: () => void
}

export const IdpProviderConfirmModal = ({
  target,
  configuredEnvironments,
  onConfirm,
  onClose,
}: IdpProviderConfirmModalProps) => {
  ...
  const handleConfirm = () => {
    if (selectedEnvironments.length === 0) {
      setError(formatMessage(m.idpProvidersDeleteEnvironmentRequired))
      return
    }
  ...
  return (
    <Modal
      ...
      isVisible
      label={formatMessage(m.idpProvidersDeleteConfirmTitle)}
      onClose={onClose}
    ...
    >
      <Box paddingX={4}>
        <Text variant="h2" as="h2" marginBottom={2}>
          {formatMessage(m.idpProvidersDeleteConfirmTitle)}
        </Text>
        <Text marginBottom={3}>
          {formatMessage(m.idpProvidersDeleteConfirmMessage)}
        </Text>

        <Box marginBottom={3}>
          <Text variant="h4" marginBottom={2}>
            {formatMessage(m.idpProvidersDeleteSelectEnvironments)}
          </Text>
          <Box
            display="flex"
