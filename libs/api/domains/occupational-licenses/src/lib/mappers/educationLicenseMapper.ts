...
export const mapEducationLicensesResponse = (licenses: MMSLicenseResponse[], locale: Locale): Array<License> =>
  {
    return licenses.map((l) => mapEducationLicense(l, locale)).filter(isDefined) ?? []
  }


export const mapEducationLicense = (data: MMSLicenseResponse, locale: Locale): License => {
