...
export const generateMockItemization = (invoiceId: number, totalAmount: number): InvoiceItem[] => {
  const numItems = Math.floor(Math.random() * 3) + 2 // 2-4 items
  const selectedItems = MOCK_ITEM_LABELS.sort(() => 0.5 - Math.random()).slice(0, numItems)

  // Generate random percentages that add up to 100%
  const percentages = Array(numItems).fill(0).map(() => Math.random())
  ...
  const itemization = selectedItems.map((label, index) => {
    ...
    const amount = isLast ? remainingAmount : Math.round(totalAmount * normalizedPercentages[index])
