...
const mapDoctor = (doctorInfo?: TrWebExternalModelsServicePortalDoctorInfo): Doctor | undefined => {
...
const mapDisabilityDiagnosis = (diagnosis?: TrWebExternalModelsServicePortalDisabilityDiagnosis): DisabilityDiagnosis | undefined => {
...
const mapDisabilityDiagnosisCollection = (
  ...
  return {
    mainDiagnosis: diagnosis?.[0] ? mapDisabilityDiagnosis(diagnosis[0]) : undefined,
    otherDiagnoses: diagnosesOthers?.map(mapDisabilityDiagnosis).filter(Boolean) as DisabilityDiagnosis[] | undefined,
  };
};

const mapHealthImpact = (healthImpact?: TrWebExternalModelsServicePortalHealthImpact): HealthImpact | undefined => {
...
const mapAbilityRating = (rating?: TrWebExternalModelsServicePortalAbilityRating): AbilityRating | undefined => {
...
const mapFunction = (func?: TrWebExternalModelsServicePortalFunction): MedicalDocumentFunction | undefined => {
...
const mapImpairmentType = (type?: TrWebExternalModelsServicePortalImpairmentType): ImpairmentType | undefined => {
...
const mapImpairment = (impairment?: TrWebExternalModelsServicePortalImpairment): Impairment | undefined => {
  if (!impairment) return undefined;

  return {
    type: mapImpairmentType(impairment.type),
    functions: impairment.functions?.map(mapFunction).filter(Boolean) as MedicalDocumentFunction[] | undefined,
  };
};

const mapEnvironmentalCategory = (category?: TrWebExternalModelsServicePortalEnvironmentalCategory): EnvironmentalCategory | undefined => {
...
const mapEnvironmentalFactor = (factor?: TrWebExternalModelsServicePortalEnvironmentalFactor): EnvironmentalFactor | undefined => {
...
export const mapDisabilityPensionCertificate = (data: TrWebExternalModelsServicePortalDisabilityPensionCertificate): DisabilityPensionCertificate | null => {
  ...
  return {
    ...
    diagnoses: mapDisabilityDiagnosisCollection(data.diagnosis, data.diagnosesOthers),
    healthHistorySummary: data.healthHistorySummary ?? undefined,
    healthImpact: mapHealthImpact(data.healthImpact),
    participationLimitationCause: data.participationLimitationCause ?? undefined,
    ...
    physicalAbilityRatings: data.physicalAbilityRatings?.map(mapAbilityRating).filter(Boolean) as AbilityRating[] | undefined,
    cognitiveAndMentalAbilityRatings: data.cognitiveAndMentalAbilityRatings?.map(mapAbilityRating).filter(Boolean) as AbilityRating[] | undefined,
    functionalAssessment: data.functionalAssessment?.map(mapAbilityRating).filter(Boolean) as AbilityRating[] | undefined,
    impairments: data.impairments?.map(mapImpairment).filter(Boolean) as Impairment[] | undefined,
    environmentalFactors: data.environmentalFactors?.map(mapEnvironmentalFactor).filter(Boolean) as EnvironmentalFactor[] | undefined,
  };
};
