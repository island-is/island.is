 ...
 */
export function addNextRedirectToUrl(url: string, nextRedirect: string): string {
 ...
 */
export interface PartialNextAuthDefaultJWT extends Pick<DefaultJWT, "email" | "name" | "picture"> {
    idToken?: string
    accessToken: string
    refreshToken: string
    ...
    userId: string
    id?: string
    idp: "apple" | "azure-ad" | "b2c-facebook" | "facebook" | "github" | "google" | "keycloak" | "twitter"
}
 ...
 */
export function extractUserPrincipalName(userOrSession: Partial<User | { idToken?: string } & Partial<Session>>) {
    if ("user" in userOrSession) {
        return userOrSession.user?.email || null
    } else if ("idToken" in userOrSession) {
        const idTokenDecoded = Buffer.from(userOrSession.idToken!.split(".")[1], "base64").toString("utf8")
