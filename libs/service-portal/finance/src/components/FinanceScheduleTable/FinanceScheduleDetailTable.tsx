...
import * as styles from '../FinanceStatusDetailTable/FinanceStatusDetailTable.css'
import { tableStyles } from '@island.is/service-portal/core'

interface Props {
  organization: any
  financeStatusDetails: any
  downloadURL: string
  userInfo: User
}

const FinanceScheduleDetailTable: FC<Props> = ({
  organization,
  ...
  return (
    <Box className={styles.wrapper} background="white">
      <T.Table>
        ...
        <T.Body>
          {financeStatusDetails?.chargeItemSubjects?.map((row: any, i: any) => (
            <T.Row key={i}>
              {[
