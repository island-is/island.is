...
describe('AwsService', () => {
    ...
    it.each([
        ...
        const result = await awsService.getFileContent({bucket: 'abc', key: 'def'}, encoding as EncodingString)
    ...
    it('should return a valid presigned url', async () => {
        ...
        const result = await awsService.getPresignedUrl({bucket: 'x', key: 'y'}, 100)
    ...
    it.each([404, 403, 500])('should return false if the object doesnt exist in s3', async (code) => {
    ...
    it.each([200, 204])('should resolve without error if file was deleted in s3', async (code) => {
        const metadata = { httpStatusCode: code}
    ...
    it.each([201, 301, 404, 500])('should log error if file was not deleted in s3', async (code) => {
        const metadata = { httpStatusCode: code}
        s3Mock.on(DeleteObjectCommand).resolvesOnce({$metadata: metadata})
