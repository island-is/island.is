...
@Injectable()
export class HolarUniversityApplicationClient {
  constructor(
  ...
  async getCourses(programExternalId: string): Promise<ICourse[]> {
    ...
    return mapUglaCourses(res, (courseExternalId: string, e: Error) => {
      logger.error(
        `Failed to map course with externalId ${courseExternalId} for program with externalId ${programExternalId} (holar-university), reason:`,
        e,
      )
    })
  }
