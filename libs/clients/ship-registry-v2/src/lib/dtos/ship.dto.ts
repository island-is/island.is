...
import { mapShipCertificate, type ShipCertificateDto } from './shipCertificate.dto'
...
export interface ShipDetailDto
  extends Omit<MyShipDetailDto, 'shipRegistrationInfo' | 'shipCertificateDetails'> {
...
export const mapShipDetail = (dto: MyShipDetailDto): ShipDetailDto => {
  ...
  const seaworthyExpiryDateParsed =
    info?.seaworthyExpiryDate?.value && info.seaworthyExpiryDate.value !== '-'
      ? parseDate(info.seaworthyExpiryDate.value, 'yyyy-MM-dd HH:mm:ss') ?? undefined
      : undefined
