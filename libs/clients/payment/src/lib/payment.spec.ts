...
import { Catalog, Item } from './payment.type'
...
const domain = 'https://tbrws-s.hysing.is/chargeFJS/v1'
...
const handlers = [
  ...
  rest.get<Catalog>(paymentUrl('/catalog'), (req, res, ctx) => {
    const { params } = req
...
describe('getCatalog', () => {
  ...
  it('should throw on error', async () => {
    await expect(paymentService.getCatalog()).rejects.toThrow()
  })
})
