...
export class OccupationalLicensesEducationController {
  ...
  async getOccupationalLicenseEducationPdf(
    ...
    console.log('from download service', user.nationalId, licenseId)
    ...
    console.log('from download service', documentResponse)
    ...
    if (documentResponse) {
      ...
      res.header(
        'Content-Disposition',
        `inline; filename=${user.nationalId}-starfsleyfi-${licenseId}.pdf`,
      )
