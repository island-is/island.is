...
export const withHealthchecks = (externalEndpointDependencies: string[]) => (
  ...
  NewComponent.getInitialProps = async (context) => {
    ...
    */
    if (context.ctx.req?.url === '/readiness') {
      ...
      context.ctx.res.statusCode = status
      context.ctx.res.end('')
      return null
    }

    // tels the cluster this instance is alive
    if (context.ctx.req?.url === '/liveness') {
      context.ctx.res.statusCode = 200
      context.ctx.res.end('')
      return null
    }
    return context.Component.getInitialProps(context)
  }
