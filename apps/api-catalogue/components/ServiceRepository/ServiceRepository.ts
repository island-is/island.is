...
const OrgServices: Array<ServiceDetails> = [
  {
    ...
    description: 'Fasteignaskráarlýsing',
    status: SERVICE_STATUS.OK,
    pricing: [PRICING_CATEGORY.FREE],
    data: [DATA_CATEGORY.PUBLIC],
    type: [TYPE_CATEGORY.REST],
    access: [ACCESS_CATEGORY.X_ROAD],
  },
  {
    id: '1',
    owner: 'Þjóðskrá',
    name: 'Einstaklingsskrá',
    url: 'http://einstaklingskra.thodskra.is:4700',
    description: 'Einstaklingarskráarlýsing',
  ...
  },
  {
    ...
    owner: 'Þjóðskrá',
    name: 'Staðfangaskrá',
    url: 'http://stadfangaskra.thodskra.is:4700',
    description: 'Staðfangaskráarlýsing',
  ...
  },
  {
    ...
    url: 'http://vsk.skattur.is/:2100',
    description: 'Virðisaukaskattarlýsing',
    status: SERVICE_STATUS.WARNING,
    pricing: [PRICING_CATEGORY.PAID],
  ...
  },
  {
    ...
    owner: 'Skatturinn',
    name: 'Staðgreiðsla',
    url: 'http://stadgreidsla.skattur.is:2100',
    description: 'Staðgreiðslulýsing',
    status: SERVICE_STATUS.OK,
  ...
  {
    id: '5',
    owner: 'Vinnumálastofnun',
    name: 'Fæðingarorlofssjóður',
    url: 'http://faedingarorlofssjodur.vms.is:74200',
    description: 'Fæðingarorlofssjóðslýsing',
    status: SERVICE_STATUS.ERROR,
    pricing: [PRICING_CATEGORY.PAID],
    data: [DATA_CATEGORY.PERSONAL, DATA_CATEGORY.PUBLIC],
    type: [TYPE_CATEGORY.REST],
  ...
  },
  {
    ...
    owner: 'Samgöngustofa',
    name: 'Ökutækjaskrá',
    url: 'http://okutaeki.samgongustofa.is:74200',
    description: 'Ökutækjaskráarlýsing',
  ...
  },
  {
    id: '7',
    owner: 'Dúddi í bæ',
    name: 'Free and paid service',
    url: 'http://asdf.asdf:74200',
    description: 'MonthlyFreeServiceLýsing',
    status: SERVICE_STATUS.UNKNOWN,
    pricing: [PRICING_CATEGORY.FREE, PRICING_CATEGORY.PAID],
    data: [
...
const ArrayContainsOneOrMoreOf = (
  checkMe: Array<string>,
  shouldContainOneOf: Array<string>,
): boolean => {
  if (
    shouldContainOneOf === undefined ||
    shouldContainOneOf === null ||
    shouldContainOneOf.length < 1
  ) {
    return true
  }
...
const ParameterArraysContainsOneOrMoreOf = (
  service: ServiceCardInformation,
  parameters: GetServicesParameters,
): boolean => {
  return (
    ArrayContainsOneOrMoreOf(service.pricing, parameters.pricing) &&
...
export async function getServices(
  ...
  if (limited.result === null || limited.result.length < 1) return limited
...
async function limitServices(
  services,
  cursor: string,
  limit: number,
): Promise<ServicesResult> {
  await timeout(1000)
  const errorResult: ServicesResult = {
    result: null,
    prevCursor: null,
    nextCursor: null,
  }
  ...
  const safeLimit =
    limit === null || limit < 0 || limit > MAX_LIMIT ? MAX_LIMIT : limit

  if (len < 1) {
    return errorResult
  }
  const startIndex =
    cursor === null || !isValidId(cursor)
      ? 0
      : services
          .map(function (x) {
            return x.id
          })
          .indexOf(searchCursor)
