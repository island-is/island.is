...
export class JwtToken {
	...
	public signJwt(secret: string): Promise<string> {
		return new Promise<string>((resolve, reject) => {
			jwt.signJWT(this, secret, 'HS256',  function (err: string, token: string | PromiseLike<string>) {
				if(err) reject(err)
				resolve(token)
			})
		})
...
export class JwtUtils {
	static async isValidJwt(token: string, secret: string): Promise<boolean> {
		return new Promise<boolean>((resolve, reject) => {
			jwt.verifyJWT(token, secret, 'HS256',  function (err: any, isValid: boolean | PromiseLike<boolean>) {
				if(err) reject(err)
				resolve(isValid)
			})
		})
