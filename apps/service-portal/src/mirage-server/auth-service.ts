...
export class AuthService {
	...
	public getActorByNationalId(nId: string): Actor {
		return this.db.actors.findBy((x: { nationalId: string }) => x.nationalId === nId)
	}

	public getSubjectByNationalId(nId: string): Subject {
		return this.db.subjects.findBy((x: { nationalId: string }) => x.nationalId === nId)
	}
	...
	public getSubjectForActor(actor: Actor) : Subject {
		const availableSubjectEntities: Subject[] = actor.subjectIds.map((x: number) => {
			return this.db.subjects.find(x) as Subject
		})
		const subjectEntity = availableSubjectEntities.find(x => x.nationalId === actor.nationalId)
