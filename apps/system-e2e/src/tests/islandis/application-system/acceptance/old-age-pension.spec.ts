...
import { oldAgePensionFormMessage } from '@island.is/application/templates/old-age-pension'
...
applicationTest.describe('Old Age Pension', () => {
  applicationTest(
    ...
    async ({ applicationPage }) => {
      ...
      // For information sub section
      await expect(
        page.getByRole('heading', {
          name: label(oldAgePensionFormMessage.pre.forInfoSubSection),
        }),
      ).toBeVisible()
      await proceed()
      ...
      await page
        .getByRole('region', {
          name: label(oldAgePensionFormMessage.payment.spouseAllowance),
        })
        .getByRole('radio', {
          name: label(oldAgePensionFormMessage.shared.yes),
        })
      ...
      const spouseAllowance = page.getByTestId('spouse-allowance-usage')
      await spouseAllowance.selectText()
      await spouseAllowance.type('100')
      ...
      // TODO: Should we select connected applications?
      await expect(
        page.getByRole('heading', {
          name: label(
      ...
      await expect(
        page.getByRole('heading', {
          name: label(oldAgePensionFormMessage.fileUpload.additionalFileTitle),
        }),
      ).toBeVisible()
      await proceed()
      ...
      await expect(
        page
          ...
          .getByRole('paragraph')
          .filter({
            hasText: label(oldAgePensionFormMessage.review.confirmSectionTitle),
          }),
      ).toBeVisible()
      await page
        .getByRole('button', {
