...
const makeEmailAccount = async (name: string) => {
    ...
    const testAccount = emailAccountExists ? JSON.parse(readFileSync(storage, {encoding: 'utf-8'})) : await createTestAccount()
    ...
    writeFileSync(storage, JSON.stringify({user: testAccount.user, pass: testAccount.pass}), {encoding: 'utf-8'})
