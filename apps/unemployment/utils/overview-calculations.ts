...
export function makeCalculations(params: CalculationParameters) {
  ...
  if (
    params.FormerSalary * constants.PercentOfSalaryConnected >=
    constants.MaximumUnemploymentBenefitSalaryConnected
  ) {
    AmountSalaryConnectedBenefits =
      constants.MaximumUnemploymentBenefitSalaryConnected
  } else if (
    params.FormerSalary * constants.PercentOfSalaryConnected >=
    constants.BaseUnemploymentBenefitAmount
  ) {
    AmountSalaryConnectedBenefits =
      params.FormerSalary * constants.PercentOfSalaryConnected
  }
  ...
  if (
    params.DateOfFirstUnemploymentPayment.valueOf <=
    constants.DateUntilSalaryConnectedBenefitsIsSixMonths.valueOf
  ) {
  ...
  MonthsBetween =
    ...
    params.DateOfFirstUnemploymentPayment.getMonth() +
    12 *
      (params.DateOfCurrentUnemploymentInterval.getFullYear() -
        params.DateOfFirstUnemploymentPayment.getFullYear())
  if (
    AmountSalaryConnectedBenefits == constants.BaseUnemploymentBenefitAmount ||
    MonthsBetween > NumberOfMonthsWithBenefitSalaryConnected
  ) {
    UnemploymentBenefitAmout = constants.BaseUnemploymentBenefitAmount
    BenefitAmountForChildren =
      params.NumberOfChildren *
      constants.BaseUnemploymentBenefitAmount *
      constants.PercentPerChildWithBaseUnemploymentBenefits
  } else if (
    params.DateOfCurrentUnemploymentInterval.valueOf >
      params.DateOfFirstUnemploymentPayment.valueOf ||
    params.DateOfCurrentUnemploymentInterval.valueOf <
      params.DateOfFirstUnemploymentPayment.valueOf
  ) {
    ...
    BenefitAmountForChildren =
      params.NumberOfChildren *
      constants.BaseUnemploymentBenefitAmount *
      constants.PercentPerChildWithSalaryConnectedBenefits
  } else {
    UnemploymentBenefitAmout =
      (constants.BaseUnemploymentBenefitAmount +
        AmountSalaryConnectedBenefits) /
      2
    BenefitAmountForChildren =
  ...
  UnemploymentBenefitAmout =
    UnemploymentBenefitAmout * (1 - params.PercentageOfPartialJob / 100.0)

  BenefitDeduction =
    (UnemploymentBenefitAmout +
      params.PartialJobPayment +
      params.SocialSecurityBenefits +
      params.PensionPayment +
      params.CapitalGainsPayment -
      (constants.BaseUnemploymentBenefitAmount +
        constants.MaximumAllowedEarningsWithoutBenefitCut)) /
    2
