...
describe(`${INDICTMENTS_COURT_OVERVIEW_ROUTE}/:id`, () => {
  ...
  it('should display relevant data', () => {
    cy.getByTestid('infoCard').contains(
      `${caseData.defendants && caseData.defendants[0].name}, kt. ${
        caseData.defendants && caseData.defendants[0].nationalId
      }, ${caseData.defendants && caseData.defendants[0].address}`,
    )
