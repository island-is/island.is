  ...
  switch (action.type) {
    ...
    case 'changeName': {
      ...
      if (newActive.type === 'Step') {
        const newSteps: IStep[] = steps.map((s) =>
          s.guid === newActive.data.guid ? (newActive.data as IStep) : s,
        )
        return {
          ...lists,
        ...
        const newGroups: IGroup[] = groups.map((g) =>
          g.guid === newActive.data.guid ? (newActive.data as IGroup) : g,
        )
        return {
          ...lists,
          activeItem: newActive,
        ...
        const newInputs: IInput[] = inputs.map((i) =>
          i.guid === newActive.data.guid ? (newActive.data as IInput) : i,
        )
        return {
          ...lists,
          activeItem: newActive,
