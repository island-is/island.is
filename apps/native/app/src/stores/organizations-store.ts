...
export const organizationsStore = create<OrganizationsStore>(persist((set, get) => ({
  ...
  getOrganizationLogoUrl(forName: string, size = 100) {
    ...
    if (!c) {
      ...
      const match = orgs.find(o => o.query === qs) || orgs.find(o => o.logo?.title === 'Skjaldarmerki');
      c = match?.logo?.url;
      if (c) {
        logoCache.set(forName, c);
      }
    }
    const url = c ?? '//images.ctfassets.net/8k0h54kbe6bj/6XhCz5Ss17OVLxpXNVDxAO/d3d6716bdb9ecdc5041e6baf68b92ba6/coat_of_arms.svg';
  ...
  actions: {
    updateOriganizations() {
      client.query({ query: GET_ORGANIZATIONS_QUERY }).then(res => {
        set({ organizations: processItems(res.data.getOrganizations.items) });
      });
    }
  }
}), {
