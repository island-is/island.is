...
const Text = styled.Text<{ isTransparent?: boolean, isOutlined?: boolean; disabled?: boolean; }>`
  ${font({
    fontWeight: '600',
    color: (props) => props.isTransparent && props.disabled ? props.theme.color.dark200 :
...
export function Button({ title, isTransparent, isOutlined, icon, ...rest }: ButtonProps) {
  ...
  return (
    ...
    >
      <>
        <Text isTransparent={isTransparent} isOutlined={isOutlined} disabled={rest.disabled}>{title}</Text>
