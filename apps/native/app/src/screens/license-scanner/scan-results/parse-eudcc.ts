...
export async function parseEudcc(data: string | any): Promise<DecodedEudcc> {
  // remove header bytes
  ...
  const { kid, cert, country, expiresAt, algo } = decodeCbor(rawData)
  ...
  if (publicKey) {
    const keyOffset = 307
    const x = Buffer.from(publicKey.rawData, 'base64').slice(
      keyOffset + 1,
      keyOffset + 1 + 32,
    )
    const y = Buffer.from(publicKey.rawData, 'base64').slice(
      keyOffset + 33,
      keyOffset + 33 + 32,
    )
    const verifyPayload = {
    ...
    const verified = await cose.sign.verify(rawData, verifyPayload)

    if (verified.equals(schema.value[2])) {
      response.verified = true;
    }
  }

  response.valid = response.verified && !response.expired;
