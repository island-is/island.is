...
export const HealthOverviewScreen: NavigationFunctionComponent = ({
  ...
  const columnGaps = theme.spacing[1] * (NUMBER_OF_CARDS_PER_ROW - 1) // Gaps between cards
  const cardWidth = (width - horizontalPadding - columnGaps) / NUMBER_OF_CARDS_PER_ROW
        ...
        {isLoadingFeatureFlags ? (
          <>
            {Array.from({ length: 2 }).map((_, index) => (
              <Row key={index}>
                {Array.from({ length: NUMBER_OF_CARDS_PER_ROW }).map(
                  (_, index) => (
                    <Skeleton
                      height={70}
