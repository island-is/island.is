...
export const WalletPassportScreen: NavigationFunctionComponent<{
  ...
  const showChildrenPassport = useFeatureFlag('isChildrenPassportEnabled', false);

  const intl = useIntl()
  const { data, loading, error } = useQuery(GET_IDENTITY_DOCUMENT_QUERY, {client, fetchPolicy: 'cache-first'})
      ...
      <Information contentInset={{ bottom: 162 }}>
        <SafeAreaView style={{ marginHorizontal: 0 }}>
          ...
          <View style={{ paddingTop: 24, paddingHorizontal: 16 }}>
            <Alert title={intl.formatMessage({ id: 'walletPassport.infoTitle' })} message={intl.formatMessage({ id: 'walletPassport.infoDescription' })} type="info" hasBorder />
          </View>

          {expireWarning ? (
            <View style={{ paddingTop: 16, paddingHorizontal: 16, paddingBottom: 10 }}>
              <Alert title={intl.formatMessage({ id: 'walletPassport.warningTitle' })} message={intl.formatMessage({ id: 'walletPassport.warningDescription' })} type="warning" hasBorder />
            </View>) : null
          }
          ...
          <InputRow>
            <Input
              ...
              value={capitalizeEveryWord(`${item?.displayFirstName} ${item?.displayLastName}`)}
              loading={loading}
              error={!!error}
              size="big"
              borderDark
          ...
          <InputRow>
            {item?.issuingDate ? (
              <Input
                ...
                value={item?.issuingDate ? intl.formatDate(new Date(item?.issuingDate)) : '-'}
                loading={loading}
                error={!!error}
                noBorder
                isCompact
            ...
            {item?.expirationDate ? (
              <Input
                ...
                value={item?.expirationDate ? intl.formatDate(new Date(item?.expirationDate)) : '-'}
                loading={loading}
                error={!!error}
                noBorder
                isCompact
                ...
                {childrenPassport?.map((child: any) => {
                  ...
                  return (
                    <AccordionItem
                      ...
                      icon={
                        <Image
                          source={isInvalid ? IconStatusNonVerified : IconStatusVerified}
                            ...
                            <View key={passport.number}>
                              ...
                              <InputRow>
                                {passport?.issuingDate ? (
                                  <Input
                                    label={intl.formatMessage({ id: 'walletPassport.issuingDate' })}
                                    value={passport?.issuingDate ? intl.formatDate(new Date(passport?.issuingDate)) : '-'}
                                    loading={loading}
                                    error={!!error}
                                    noBorder
                                    isCompact
                                ...
                                {passport?.expirationDate ? (
                                  <Input
                                    label={intl.formatMessage({ id: 'walletPassport.expirationDate' })}
                                    value={passport?.expirationDate ? intl.formatDate(new Date(passport?.expirationDate)) : '-'}
                                    loading={loading}
                                    error={!!error}
                                    noBorder
                                    isCompact
