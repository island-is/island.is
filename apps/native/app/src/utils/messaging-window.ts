// Ported from the my-pages health conversations messaging-window helper, so both
// clients derive the same labels from the same recipient fields: the opening
// hours per day type, today's window, and when the next window opens.
...
export const getOpeningHoursLabels = (
  ...
  // A recipient closed on every day type has nothing to list.
  if (!weekday && !weekend && !holiday) {
    return undefined
  }
...
export const getNextOpeningInfo = (
  ...
  if (!timeLabel || isNaN(date.getTime())) {
    return undefined
  }
...
export interface ClosedRecipient {
  openingHours?: OpeningHours | null
  nextOpensAt?: NextOpening | null
}
