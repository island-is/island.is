...
storiesOf('Alert', module)
  ...
  .add('Info', () => {
    const message = text('Info Alert meassage', 'Til að nota skírteini sem gild skilríki þarf að færa þau yfir í Apple Wallet.');

    return (
      <Alert key="alert1" type="info" message={message} hideIcon={boolean('Hide Info Icon', false)} />
    );
  })
  .add('Info No Icon', () => {
    const message = text('Info Alert meassage', 'Til að nota skírteini sem gild skilríki þarf að færa þau yfir í Apple Wallet.');
    ...
    return (
      <Alert key="alert2" type="info" message={message} hideIcon={true} />
    );
  })
  .add('Success', () => {
