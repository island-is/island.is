...
const SEARCH_KEYS: (keyof TaxiDriver)[] = ['name']

interface TaxiDriversWithWorkPermitListProps {
  slice: ConnectedComponent
}

type ListState = 'loading' | 'loaded' | 'error'

const TaxiDriversWithWorkPermitList = ({
  slice,
}: TaxiDriversWithWorkPermitListProps) => {
  ...
  useQuery<Query>(GET_TAXI_DRIVERS_WITH_WORK_PERMIT_QUERY, {
    onCompleted: (data) => {
      const fetched = data?.getTaxiDriversWithWorkPermit?.drivers ?? []
      setDrivers(
  ...
  return (
    <Box>
      ...
      {listState === 'loaded' && (
        <Box marginBottom={4}>
          <Input
            name="taxiDriverWorkPermitSearch"
...
export default TaxiDriversWithWorkPermitList
