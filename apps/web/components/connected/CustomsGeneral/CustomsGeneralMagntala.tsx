...
import { GET_CUSTOMS_GENERAL_MAGNTALA } from '@island.is/web/screens/queries/CustomsGeneral'

import { CustomsGeneralDateTable, toApiDate } from './CustomsGeneralDateTable'
import { m } from './translation.strings'

const CustomsGeneralMagntala = () => {
  ...
  const { data, loading, error } = useQuery(GET_CUSTOMS_GENERAL_MAGNTALA, {
    variables: { input: { dags: toApiDate(selectedDate), kerfi } },
  })

  const items = (data?.customsGeneralMagntala ?? []).map(
...
export default CustomsGeneralMagntala
