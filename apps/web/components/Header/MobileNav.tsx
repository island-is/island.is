          ...
          {HEADER_NAV_KEYS.map((key) => {
            ...
            return (
              <li key={key}>
                ...
                >
                  <span className={styles.drillRowLabel}>{section.label}</span>
                  <Icon
                    icon="chevronForward"
                    type="outline"
                    size="small"
...
>(function MobileNavSearchButton({ onClick }, ref) {
...
>(function MobileNavMenuButton({ onClick, isOpen }, forwardedRef) {
  const { activeLocale } = useI18n()
  const menuLabel = activeLocale === 'is' ? 'Valmynd' : 'Menu'
