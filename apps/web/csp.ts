 ...
 * (next/dist/server/render.js `getScriptNonceFromHeader`) and stamps it onto all
 * its inline scripts — pages router included. This is done in the custom server
 * rather than Next middleware because middleware request-header overrides do not
 * reach the pages renderer through the Express custom server.
 *
