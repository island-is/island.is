...
export class IslykillService {
  ...
  async updateOrCreateIslykillSettings(
    nationalId: User['nationalId'],
    {
      email,
      mobile,
  ...
  ): Promise<CreateIslykillSettings | UpdateIslykillSettings> {
    const islykillData = await this.getIslykillSettings(nationalId)

    if (islykillData.noUserFound) {
      await this.createIslykillSettings(nationalId, {
        email: email && emailVerified ? email : undefined,
        mobile: mobile && mobileVerified ? mobile : undefined,
      }).catch((error) => {
        throw new BadRequestException(
          `createIslykillSettings failed in islyklar updateOrCreateIslykillSettings: ${error.message}`,
        )
      })
    } else {
      await this.updateIslykillSettings(nationalId, {
        email: email && emailVerified ? email : islykillData.email,
        mobile: mobile && mobileVerified ? mobile : islykillData.mobile,
        bankInfo: bankInfo ?? islykillData.bankInfo,
        canNudge:
          typeof canNudge === 'boolean' ? canNudge : islykillData.canNudge,
      }).catch((error) => {
        throw new BadRequestException(
          `updateIslykillSettings failed in islyklar updateIslykillSettings: ${error.message}`,
        )
      })
    }
