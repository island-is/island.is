...
module.exports = {
  up: async (queryInterface, Sequelize) => {
      ...
      for(const duplicateEndorsementGroup of duplicateEndorsementGroups) {
        ...
        // Collect ids
        const problematicIds = duplicateEndorsementLists.map(endorsement => endorsement.id)
        ...
        if(!aggregateId || problematicIds.length == 0) {
          console.log("Nothing to be done for", JSON.stringify({
            description: duplicateEndorsementGroup.description, 
            tags: duplicateEndorsementGroup.tags,
            originalIds: duplicateEndorsementLists.map(endorsement => endorsement.id)})
          )
        }
