...
describe('EndorsementListService', () => {
  ...
  describe('populateOwnerNamesForExistingLists', () => {
    it('should populate owner names for lists without an ownerName', async () => {
      const lists = [
        { ownerNationalId: '1234567890', ownerName: '', save: jest.fn() },
      ]
  ...
  describe('findSingleList', () => {
    ...
    it('should throw NotFoundException if list is not found', async () => {
      ...
      await expect(service.findSingleList(listId)).rejects.toThrow(
        NotFoundException,
      )
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'This endorsement list does not exist.',
      )
    })
  })
  ...
  describe('close', () => {
    it('should close an endorsement list and return it', async () => {
      ...
      expect(endorsementList.update).toHaveBeenCalledWith({
        closedDate: expect.any(Date),
      })
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Closing endorsement list: ${endorsementList.id}`,
      )
    })
  })
  ...
  describe('open', () => {
    it('should open an endorsement list and update closed date', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Opening endorsement list: ${endorsementList.id}`,
      )
    })
  })
  ...
  describe('lock', () => {
    it('should lock an endorsement list and return it', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Locking endorsement list: ${endorsementList.id}`,
      )
    })
  ...
  describe('unlock', () => {
    it('should unlock an endorsement list and return it', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Unlocking endorsement list: ${endorsementList.id}`,
      )
    })
  })
  ...
  describe('updateEndorsementList', () => {
    it('should update an endorsement list with new data and return it', async () => {
      ...
      endorsementList.update.mockResolvedValue({
        ...endorsementList,
        ...newData,
      })

      const result = await service.updateEndorsementList(
        endorsementList,
        newData,
      )
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Updating endorsement list: ${endorsementList.id}`,
      )
    })
  })
  ...
  describe('create', () => {
    ...
    it('should throw BadRequestException if closedDate has already passed', async () => {
      ...
      jest
        .spyOn(global, 'Date')
        .mockImplementation(() => new Date('2024-01-01T00:00:00Z') as any)
    ...
    it('should create a new endorsement list and return it', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Creating endorsement list: Test List',
      )
    })
  })
  ...
  describe('getOwnerName', () => {
    it('should return the owner name if found in National Registry', async () => {
      ...
      expect(mockNationalRegistryService.getName).toHaveBeenCalledWith(
        ownerNationalId,
      )
    })
    ...
    it('should throw BadRequestException if National Registry lookup fails', async () => {
      ...
      await expect(service.getOwnerName(ownerNationalId)).rejects.toThrow(
        BadRequestException,
      )
  ...
  describe('emailPDF', () => {
    ...
    it('should throw NotFoundException if list is not found', async () => {
      ...
      await expect(
        service.emailPDF(listId, 'test@example.com'),
  ...
  describe('exportList', () => {
    it('should export a list as a PDF or CSV and return a presigned URL', async () => {
      ...
      mockEndorsementListModel.findOne.mockResolvedValue(mockList)
      mockAwsService.getPresignedUrl.mockResolvedValue(
    ...
    it('should throw BadRequestException for invalid file type', async () => {
      ...
      await expect(service.exportList(listId, user, fileType)).rejects.toThrow(
        BadRequestException,
      )
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Failed to export list some-list-id',
        expect.any(Object),
      )
    })
    ...
    it('should throw NotFoundException if list is not found or access denied', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Failed to export list non-existent-list-id',
        expect.any(Object),
      )
    })
  })
  ...
  describe('getOwnerContact', () => {
    ...
    it('should throw NotFoundException if contact information is not found', () => {
      ...
      expect(() => service.getOwnerContact(obj, search)).toThrow(
        NotFoundException,
      )
      expect(mockLogger.warn).toHaveBeenCalledWith(
