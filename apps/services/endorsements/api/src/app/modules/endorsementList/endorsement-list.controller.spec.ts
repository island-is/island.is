...
import { EndorsementList } from './endorsementList.model'
import { ScopesGuard } from '@island.is/auth-nest-tools'
import { ParseUUIDPipe, BadRequestException, ForbiddenException, UnauthorizedException } from '@nestjs/common'
import { EndorsementListByIdPipe } from './pipes/endorsementListById.pipe'
...
describe('EndorsementListController', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      .overrideGuard(ScopesGuard)
      .useValue({ canActivate: jest.fn((context) => {
  ...
  describe('findOne', () => {
    const validEndorsementList = { id: 'valid-uuid', name: 'Test List' } as unknown as EndorsementList
