...
export class ApplicationsService {
  ...
  async completePaymentSection(applicationId: string): Promise<void> {
    ...
    const paymentSection = form.sections.find(
      (s) => s.sectionType === SectionTypes.PAYMENT,
    )
    if (!paymentSection?.id) {
      return
    }
    const completedArray = application.completed ?? []
    if (!completedArray.includes(paymentSection.id)) {
      application.completed = [...completedArray, paymentSection.id]
      await application.save()
    }
  }
