...
export class PaymentService {
   ...
   * 1. Checks for existing payment for the application
   * 2. If payment exists, returns existing payment URL
   * 3. If no payment exists, creates new payment entry and generates payment URL
   */
  async createCharge(
    user: User,
    performingOrganizationID: string,
    ...
    //1. Fetch existing payment if any
  ...
  private async getReturnUrls(applicationId: string) {
    const slug = await this.applicationsService.getSlugFromId(applicationId)

    const returnUrl = new URL(this.config.clientLocationOrigin)
    returnUrl.pathname = `form/${slug}/${applicationId}`
  ...
  async delete(applicationId: string, user: User) {
    ...
    const applicants = await Value.findAll({
      where: {
        applicationId: applicationId,
        fieldType: 'APPLICANT',
      },
    ...
    const isCurrentUser = applicants.some((applicant) => {
      const applicantNationalId = applicant.json?.nationalId
      const isLoggedInUser = applicant.json?.isLoggedInUser
      return isLoggedInUser && applicantNationalId === nationalId
    })
