...
import { Agent, ApplicantTypesEnum, IndividualApplicant, LegalEntity } from '@island.is/form-system/ui'
...
export const Applicants = ({ applicantTypes }: Props) => {
  ...
  const agentType = ApplicantTypesEnum.INDIVIDUAL_WITH_DELEGATION_FROM_INDIVIDUAL || ApplicantTypesEnum.INDIVIDUAL_WITH_DELEGATION_FROM_LEGAL_ENTITY
  return (
    <>
      {applicantTypes.map(applicantType => {
        if (applicantType.applicantTypeId === ApplicantTypesEnum.INDIVIDUAL) {
          return <IndividualApplicant applicantType={applicantType} lang={lang} />
        } else if (applicantType.applicantTypeId === agentType) {
          return <Agent applicantType={applicantType} lang={lang} />
        } else if (applicantType.applicantTypeId === ApplicantTypesEnum.LEGAL_ENTITY) {
