...
function createTmpTsConfig(tsconfigPath, workspaceRoot, projectRoot) {
  var tmpTsConfigPath = path_1.join(
    workspaceRoot,
    'tmp',
    projectRoot,
  ...
  process.on('exit', function () {
    return cleanupTmpTsConfigFile(tmpTsConfigPath)
  })
...
function readTsConfigWithCommonJs(tsConfig, generatedTsConfigPath) {
  ...
  generatedTsConfig.extends = path_1.relative(
    path_1.dirname(generatedTsConfigPath),
    tsConfig,
  )
