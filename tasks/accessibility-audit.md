...
- [x] **S2. Select-all never showed the indeterminate state** — **FIXED**
      ...
      partial selection rendered and announced as _unchecked_ — indistinguishable
      ...
      `indeterminate: false`, `aria-checked: null`, AX tree `checkbox, checked: false` (4.1.2).
...
- [ ] **B4. The "kr." placeholder fails contrast**
      **Measured**: `rgb(153, 153, 177)` on white = **2.78:1** at 14 px; 1.4.3 needs
      ...
      Fixing B3 did remove the placeholder's _labelling_ role, which was the part
...
- [ ] **M7. Four tab stops per row, one of which is not a control**
      **Measured** tab order: `select-all → checkbox → expand toggle → (truncated cell span) → checkbox → expand toggle → …`. Ariakit's
